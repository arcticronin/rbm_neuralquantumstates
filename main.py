...
def run_vmc_study(sizes: List[int], fields: List[float], m_ratios: List[float]) -> Dict:
        ...
        if L_data:
            ...
            for g in config.TRANSVERSE_FIELDS:
                ...
                if g_data:
                    ...
                    print(
                        f"  Trend: {'improving' if g_data[-1][1] < g_data[0][1] else 'degrading'} "
...
def main():
    ...
    # Run VMC with RBM
    vmc_results = run_vmc_study(
        config.SYSTEM_SIZES, config.TRANSVERSE_FIELDS, config.HIDDEN_UNIT_RATIOS
    )
